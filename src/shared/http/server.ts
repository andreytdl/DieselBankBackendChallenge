...
import UserBalanceGetter from '@modules/users/services/UserBalanceGetter';
...
app.post('/notify', async (request, response) => {
    ...
    await binoBankReconciliationService.execute(
        binoBank,
        providers,
    );
...
app.get('/balance', (request, response) => {
    const userBalanceGetter = new UserBalanceGetter()
    const balance = userBalanceGetter.execute(binoBank.getUser())
